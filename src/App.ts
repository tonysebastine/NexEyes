...
export class App {
  ...
  constructor(containerId: string) {
    ...
    const PANEL_SPANS_KEY = 'godseye-panel-spans';

    // One-time migration: copy 'worldmonitor-*' keys → 'godseye-*' to preserve user settings after rename.
    if (!localStorage.getItem('godseye-migration-v1')) {
      const WM_SUFFIX_MAP: Record<string, string> = {
        'worldmonitor-panels': STORAGE_KEYS.panels,
        'worldmonitor-monitors': STORAGE_KEYS.monitors,
        'worldmonitor-layers': STORAGE_KEYS.mapLayers,
        'worldmonitor-disabled-feeds': STORAGE_KEYS.disabledFeeds,
        'worldmonitor-live-channels': STORAGE_KEYS.liveChannels,
        'worldmonitor-map-mode': STORAGE_KEYS.mapMode,
        'worldmonitor-variant': 'godseye-variant',
        'worldmonitor-theme': 'godseye-theme',
        'worldmonitor-panel-spans': PANEL_SPANS_KEY,
      };
      for (const [oldKey, newKey] of Object.entries(WM_SUFFIX_MAP)) {
        const val = localStorage.getItem(oldKey);
        if (val !== null && localStorage.getItem(newKey) === null) {
          localStorage.setItem(newKey, val);
        }
      }
      localStorage.setItem('godseye-migration-v1', 'done');
    }
    ...
    const storedVariant = localStorage.getItem('godseye-variant');
    const currentVariant = SITE_VARIANT;
    if (storedVariant !== currentVariant) {
      // Variant changed - use defaults for new variant, clear old settings
      localStorage.setItem('godseye-variant', currentVariant);
      ...
      const PANEL_ORDER_MIGRATION_KEY = 'godseye-panel-order-v1.9';
      ...
      if (currentVariant === 'tech') {
        const TECH_INSIGHTS_MIGRATION_KEY = 'godseye-tech-insights-top-v1';
    ...
    const LAYOUT_RESET_MIGRATION_KEY = 'godseye-layout-reset-v2.5';
  ...
  public async init(): Promise<void> {
    ...
    // Parallelize core initialization (fetching data, indexeddb, i18n)
    await Promise.all([
      initDB(),
      initI18n(),
      fetchBootstrapData(),
    ...
    if ('requestIdleCallback' in window) {
      window.requestIdleCallback(() => initML());
    } else {
    ...
    // (fetchBootstrapData is now running in parallel earlier)
    ...
    // Do not block loadAllData on country geometry; load it in background
    preloadCountryGeometry().catch(console.error);
