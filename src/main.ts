...
Sentry.init({
  ...
  release: `godseye@${__APP_VERSION__}`,
...
if (import.meta.env.DEV) {
  // Debug helpers for geo-convergence testing (dev only)
  ...
  Object.defineProperty(window, 'beta', {
    get() {
      const on = localStorage.getItem('godseye-beta-mode') === 'true';
      console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
      return on;
    },
    set(v: boolean) {
      if (v) localStorage.setItem('godseye-beta-mode', 'true');
      else localStorage.removeItem('godseye-beta-mode');
      location.reload();
    },
  });
}
