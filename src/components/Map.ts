...
const LAYER_GROUPS: LayerGroupConfig[] = [
  {
    id: 'geopolitical',
    labelKey: 'components.deckgl.layerGroup.geopolitical',
    layers: ['conflicts', 'hotspots', 'sanctions', 'protests', 'ucdpEvents', 'displacement', 'iranAttacks'],
    variants: ['full', 'tech', 'finance'],
    defaultOpen: true,
  },
  {
    id: 'military',
    labelKey: 'components.deckgl.layerGroup.military',
    layers: ['bases', 'nuclear', 'irradiators', 'military', 'spaceports'],
    variants: ['full'],
    defaultOpen: true,
  },
  {
    id: 'infrastructure',
    labelKey: 'components.deckgl.layerGroup.infrastructure',
    layers: ['cables', 'pipelines', 'outages', 'datacenters', 'economic', 'waterways', 'tradeRoutes'],
    variants: ['full', 'tech', 'finance'],
  },
  {
    id: 'transport',
    labelKey: 'components.deckgl.layerGroup.transport',
    layers: ['ais', 'flights', 'gpsJamming'],
    variants: ['full'],
  },
  {
    id: 'natural',
    labelKey: 'components.deckgl.layerGroup.environment',
    layers: ['natural', 'weather', 'fires'],
    variants: ['full', 'tech', 'finance'],
  },
  {
    id: 'analysis',
    labelKey: 'components.deckgl.layerGroup.analysis',
    layers: ['ciiChoropleth', 'dayNight'],
    variants: ['full', 'tech', 'finance'],
  },
  {
    id: 'tech',
    labelKey: 'components.deckgl.layerGroup.tech',
    layers: ['startupHubs', 'cloudRegions', 'accelerators', 'techHQs', 'techEvents'],
    variants: ['tech'],
  },
  {
    id: 'finance',
    labelKey: 'components.deckgl.layerGroup.finance',
    layers: ['stockExchanges', 'financialCenters', 'centralBanks', 'commodityHubs', 'gulfInvestments'],
    variants: ['finance'],
  },
  {
    id: 'happy',
    labelKey: 'components.deckgl.layerGroup.happy',
    layers: ['positiveEvents', 'kindness', 'happiness', 'speciesRecovery', 'renewableInstallations'],
    variants: ['happy'],
  },
];

const LAYER_LABEL_KEYS: Partial<Record<keyof MapLayers, string>> = {
  ...
  positiveEvents: 'components.deckgl.layers.positiveEvents',
  kindness: 'components.deckgl.layers.kindness',
  happiness: 'components.deckgl.layers.happiness',
  speciesRecovery: 'components.deckgl.layers.speciesRecovery',
  renewableInstallations: 'components.deckgl.layers.renewableInstallations',
  dayNight: 'components.deckgl.layers.dayNight',
};

const SUMMARY_LABEL_KEYS: Record<'sufficient' | 'limited' | 'insufficient', string> = {
  sufficient: 'components.deckgl.dataFreshness.status.sufficient',
  limited: 'components.deckgl.dataFreshness.status.limited',
  insufficient: 'components.deckgl.dataFreshness.status.insufficient',
};
...
export class MapComponent {
  ...
  private layerButtons: Partial<Record<keyof MapLayers, HTMLButtonElement>> = {};
  private freshnessSummary: DataFreshnessSummary | null = null;
  private freshnessUnsub: (() => void) | null = null;
  ...
  private createLayerToggles(): HTMLElement {
    ...
    const groups = LAYER_GROUPS.filter((group) => !group.variants || group.variants.includes(SITE_VARIANT));
    groups.forEach((group) => {
      const visibleLayers = group.layers.filter((layer) => layer in this.state.layers);
      if (visibleLayers.length === 0) return;

      const detail = document.createElement('details');
      detail.className = 'layer-group';
      detail.dataset.group = group.id;
      if (group.defaultOpen) detail.open = true;

      const summary = document.createElement('summary');
      summary.className = 'layer-group-summary';
      const label = this.getGroupLabel(group.labelKey, group.id);
      ...
      visibleLayers.forEach((layer) => body.appendChild(this.buildLayerToggle(layer)));
    ...
    helpBtn.textContent = '?';
    const helpLabel = t('components.deckgl.layerGuide');
    ...
    this.updateAllLayerStatuses();
    return toggles;
  }

  private buildLayerToggle(layer: keyof MapLayers): HTMLButtonElement {
    ...
    status.className = 'layer-toggle-status layer-toggle-status-hidden';
  ...
  private getLayerLabel(layer: keyof MapLayers): string {
    if (layer === 'sanctions') {
      return t('components.deckgl.layerHelp.labels.sanctions');
    }
  ...
  private getGroupLabel(labelKey: string, fallback: string): string {
    const translation = t(labelKey);
    if (translation && translation !== labelKey) return translation;
    return fallback.replace(/-/g, ' ');
  }

  private updateAllLayerStatuses(): void {
    (Object.keys(this.layerButtons) as (keyof MapLayers)[]).forEach((layer) => this.refreshLayerStatusDot(layer));
  }

  private refreshLayerStatusDot(layer: keyof MapLayers): void {
    const btn = this.layerButtons[layer];
    if (!btn) return;
    const dot = btn.querySelector('.layer-toggle-status') as HTMLElement | null;
    if (!dot) return;
    const sourceIds = LAYER_TO_SOURCE[layer];
    if (!sourceIds || sourceIds.length === 0) {
      dot.classList.add('layer-toggle-status-hidden');
    ...
    const states = sourceIds
      ...
      .filter((value): value is DataSourceState => Boolean(value));
    if (states.length === 0) {
      dot.classList.add('layer-toggle-status-hidden');
      btn.removeAttribute('title');
      return;
    ...
    const best = states.reduce((prev, candidate) =>
      this.compareStatus(candidate.status, prev.status) < 0 ? candidate : prev
    );

    dot.classList.remove('layer-toggle-status-hidden');
    dot.dataset.status = best.status;
    dot.style.backgroundColor = getStatusColor(best.status);
    const parts = states.map(
      (state) => `${state.name}: ${dataFreshness.getTimeSince(state.id)} (${state.status})`
    );
    btn.setAttribute('title', parts.join('\n'));
  }

  private compareStatus(a: FreshnessStatus, b: FreshnessStatus): number {
  ...
  private setupFreshnessWatcher(): void {
    this.refreshFreshnessSummary();
    if (this.freshnessUnsub) return;
    this.freshnessUnsub = dataFreshness.subscribe(() => {
      this.refreshFreshnessSummary();
    });
  }

  private refreshFreshnessSummary(): void {
    this.freshnessSummary = dataFreshness.getSummary();
    this.updateFreshnessBadge();
    this.updateAllLayerStatuses();
  }

  private updateFreshnessBadge(): void {
    if (!this.statusBadge || !this.freshnessSummary) return;
    const labelKey = SUMMARY_LABEL_KEYS[this.freshnessSummary.overallStatus];
    const labelText = t(labelKey) || this.freshnessSummary.overallStatus;
    const coverage = `${this.freshnessSummary.coveragePercent}%`;
    const color = this.getSummaryStatusColor();
    this.statusBadge.dataset.status = this.freshnessSummary.overallStatus;
    this.statusBadge.innerHTML = `
      <span class="map-status-pill"></span>
      <span class="map-status-text">${labelText}</span>
      <span class="map-status-count">${coverage} coverage</span>
    `;
    this.statusBadge.style.setProperty('--badge-color', color);
  }

  private getSummaryStatusColor(): string {
    if (!this.freshnessSummary) return getCSSColor('--text-dim');
